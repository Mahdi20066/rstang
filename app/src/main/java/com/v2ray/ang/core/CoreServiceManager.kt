...
//        if (!result.status) error(result.errorMessage.ifBlank { "Failed to get V2Ray config" })
        ...
        // RSTA NG: "Auto Setting" (Fragment Settings screen) must work out of the
        // box even if the user never opens that screen, since it defaults to on.
        // Re-apply it here on every connect attempt so a fresh install behaves
        // correctly, and so it stays correct even if changed mid-session.
        ...
        // RSTA NG: if this profile routes through the local RSTA Spoof proxy
        // (server 127.0.0.1, port 40443), make sure the native spoof engine is
        // running before we let Xray dial into it. If it's not a spoof target,
        // make sure no stale spoof session is left running from a previous config.
